"""Rebuild the dashboard from scratch - add filter + chart in one update."""
import sys
...
# Step 2: Check both snapshot and core
for rt in ['snapshot', 'core']:
    r = api('/dataVisualization/findById', {"id": panel_id, "busiFlag": "dataV", "resourceTable": rt})
    d = r.get('data') or {}
    cvi = d.get('canvasViewInfo') or {}
    comps = d.get('componentData', '[]')
    if isinstance(comps, str):
        comps = json.loads(comps)
    print(f"  {rt}: version={d.get('version')}, views={len(cvi)}, components={len(comps)}")

# Get latest version from snapshot
snap = api('/dataVisualization/findById', {"id": panel_id, "busiFlag": "dataV", "resourceTable": "snapshot"})
snap_data = snap.get('data') or {}
cur_version = snap_data.get('version') or 1
print(f"\nCurrent version (snapshot): {cur_version}")
...
x_axis = [make_field(field_map[o], d, 'd') for o, d in dims if o in field_map]
y_axis = [make_field(field_map[o], d, 'q', s) for o, d, s in metrics if o in field_map]
print(f"xAxis: {len(x_axis)}, yAxis: {len(y_axis)}")

# Date filter (chart-level)
custom_filter = {
    "logic": "and",
    "items": [{
        "type": "field",
...
# Step 4: Build chart view + filter + table components
...
components = [filter_comp, chart_comp]
...
check = f'"id":"{chart_id}"'
assert check in compact_comp, "Chart ID pattern missing!"
print(f"Chart: {chart_id}, Filter: {filter_id}")
...
print(f"\n=== Updating canvas (v{cur_version} -> v{cur_version+1}) ===")
...
if r.status_code == 200:
    ...
    if resp.get('code') == 0:
        # Publish
        pub = api('/dataVisualization/updatePublishStatus', {"id": panel_id, "type": "dataV", "busiFlag": "dataV", "status": 1, "pid": 0})
        print(f"Publish: {pub.get('code')}")
        
        # Verify
        time.sleep(2)
        v = api('/dataVisualization/findById', {"id": panel_id, "busiFlag": "dataV", "resourceTable": "core"})
        vd = v.get('data', {})
        ...
        print(f"Verified: {len(vc)} components, {len(vcvi)} views")
        for vid, vi in vcvi.items():
            print(f"  View {vid}: type={vi.get('type')}, x={len(vi.get('xAxis',[]))}, y={len(vi.get('yAxis',[]))}")
