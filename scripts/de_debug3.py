"""Binary search for 400 error with native JSON types."""
import sys
...
# Try each axis field as native type
fields_to_test = [
    ("xAxis", []),
    ("yAxis", []),
    ("xAxisExt", []),
...
for key, val in fields_to_test:
    ...
    print(f"  + {key}={repr(val)}: {r.status_code} ({'OK' if status_ok else 'FAIL'})")
    if not status_ok:
        del chart[key]

# Now try with xAxis containing actual field data
print("\n=== Testing xAxis with data ===")
x_field = {
    "id": "1774862841744",
...
r = requests.post(f'{base}/de2api/dataVisualization/updateCanvas',
                  headers=headers, json=payload, timeout=15)
print(f"xAxis with field: {r.status_code}")
if r.status_code != 200:
    del chart["xAxis"]
    print(f"  Response: {r.text[:200]}")

print(f"\nWorking keys: {list(chart.keys())}")
