"""Update the existing DSP Report dashboard with our new chart."""
import sys
...
panel_id = "1236050016221663232"  # Existing DSP Report dashboard
...
print(f"Dashboard: {snap_data.get('name')}, version: {cur_version}")
print(f"Current views: {len(snap_data.get('canvasViewInfo', {}) or {})}")
...
chart_view = {
    ...
    "drillFields": [],
    "senior": {
        "functionCfg": {"sliderShow": False, "sliderRange": [0, 10]},
        "assistLineCfg": {"enable": False, "assistLine": []},
        "threshold": {"enable": False, "tableThreshold": []},
        "scrollCfg": {"open": False, "row": 1, "interval": 2000, "step": 50}
    },
    "stylePriority": "panel", "chartType": "private",
...
print(f"\n=== Updating existing DSP Report dashboard ===")
...
if r.status_code == 200:
    ...
    if resp.get('code') == 0:
        ...
        print(f"Core: {len(vcvi)} views")
        for vid, vi in vcvi.items():
            print(f"  {vid}: type={vi.get('type')}, x={len(vi.get('xAxis',[]))}, y={len(vi.get('yAxis',[]))}")
...
with sync_playwright() as p:
    ...
    time.sleep(5)
    page.screenshot(path='d:/Projects/m2/scripts/ss_final_initial.png')
    print("Initial screenshot saved")
    
    # Wait for data
    print("Waiting for data to load (30s)...")
    time.sleep(30)
    page.screenshot(path='d:/Projects/m2/scripts/ss_final_loaded.png')
