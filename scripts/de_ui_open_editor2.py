"""Click edit icon to open dashboard editor (no expect_navigation)."""
import sys
...
with sync_playwright() as p:
    ...
    page.screenshot(path='d:\\Projects\\m2\\scripts\\dash_editor_02.png')
    
    print(f"\nAPI calls: {len(api_calls)}")
    for c in api_calls:
        print(f"  {c['method']} {c['url']}")
        if c['post_data'] and ('findById' in c['url'] or 'Canvas' in c['url'] or 'tree' in c['url']):
    ...
    # Look for chart/view adding buttons in the editor
